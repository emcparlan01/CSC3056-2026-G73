...
public class SimpleBankingApp {
	...
	public static void printAllAccounts() {
	    ...
	    SimpleDateFormat sdf = new SimpleDateFormat("MMM dd, yyyy"); // Aug 20, 2019
	    ...
	    for (int i = 0; i < accounts.size(); i++) {
	        ...
	        String openingDateStr = (a.getAccount_opening_date() == null)
	                ? "null"
	                : "(" + sdf.format(a.getAccount_opening_date()) + ")";

	        System.out.println(String.format("%-10s| %-30s| %-10s| %-15s| $%-15.2f",
	                a.getAccount_number(),
	                a.getUsername_of_account_holder(),
	                a.getAccount_type(),
	                openingDateStr,
	                getBalance(a.getAccount_number())));
	    }
	}
	...
	public static double getBalance(String account_number) {
		// initial balance assumed to be 0
		...
		for (int i = 0; i < transactions.size(); i++) {
			...
			if (t != null && t.getAccountNumber() != null && t.getAccountNumber().equals(account_number)) {
	...
	// TODO12 Feature 1: accountExists
	...
	public static boolean accountExists(String account_number) {
		...
		for (int i = 0; i < accounts.size(); i++) {
			Account a = accounts.get(i);
			if (a != null && a.getAccount_number() != null && a.getAccount_number().equals(account_number)) {
				return true;
			}
		}
		return false;
	...
	// TODO12 Feature 2: addTransactionSafe 

	public static boolean addTransactionSafe(String account_number, double amount) {

		// reject if account doesn't exist
		if (!accountExists(account_number)) {
			return false;
		}

		// if valid, add transaction using existing logic
		addTransaction(account_number, amount);
		return true;
	}
	
	// TODO12 Feature 3: getTransactionsForAccount
	public static Vector<Transaction> getTransactionsForAccount(String accountNumber) {
	    ...
	    for (Transaction t : transactions) {
	        if (t != null && t.getAccountNumber() != null && t.getAccountNumber().equals(accountNumber)) {
